...
class zia:
    ...
    def __init__(self, cloud, username, password, apikey):
        ...
        self.logger = logging.getLogger('zia_api')
        self.logger.info('creating an instance of ZIA API - cloud {}'.format(cloud))
    ...
    def authenticate(self):
        self.logger.debug('authenticate module called')
        if self.jsessionid == None:
            self.logger.debug('no jsessionid found - need to run login module')
            secret_key = obfuscateApiKey(self.apikey)
            timestamp = secret_key[0]
            obf_key = secret_key[1]

            self.login(timestamp, obf_key)
            return True
        else:
            ...
            try:
                request = self._api_call(sys._getframe().f_code.co_name, api_path, method, headers=headers)
                ...
                self.logger.debug("Receieved error -{} - attempting to re-login".format(err))
                self.login(timestamp, obf_key)
            except Exception as e:
                self.logger.debug("Receieved error -{} - attempting to re-login".format(e))
                self.login(timestamp, obf_key)

    def logout(self):
        self.logger.info('logout module called')
        api_path = '/authenticatedSession'
        headers = {'Content-Type' :  'application/json', 'cache-control': 'no-cache', 'cookie' : ''}
        headers.update(cookie=self.jsessionid)
        method = 'DELETE'
        ...
        try:
            ...
            self.logger.info("Logout attempted - but no valid session found")
            
        return request.json()

    def login(self, timestamp, key):
        self.logger.debug('login module called')
        api_path = '/authenticatedSession'
        ...
        request = self._api_call(sys._getframe().f_code.co_name, api_path, method, headers=headers, body=data)
        if request:
            response_data = request.json()
            jsessionid = request.cookies['JSESSIONID']
            self.logger.debug('setting jsessionid')
            self.jsessionid = "{}{}".format('JSESSIONID=',jsessionid)

        return request.json()

    def get_groups(self, search=None, page=None, pageSize=None):
        self.logger.debug('get_groups module called')
        api_path = '/groups'
        headers = {'Content-Type' :  'application/json', 'cache-control': 'no-cache', 'cookie' : ''}
        headers.update(cookie=self.jsessionid)
        method = 'GET'
    ...
    def get_departments(self, name=None, page=None, pageSize=None):
        self.logger.debug('get_departments module called')
    ...
    def get_user(self, id):
        self.logger.debug("get_user module called")
    ...
    def add_user(self, details):
        self.logger.debug("update_user module called")
        api_path = '/users/'
        headers = {'Content-Type' :  'application/json', 'cache-control': 'no-cache', 'cookie' : ''}
        headers.update(cookie=self.jsessionid)
        method = 'POST'
    ...
    def get_users(self, name=None, dept=None, group=None, page=None, pageSize=None):
        self.logger.debug('get_users module called')
        api_path = '/users'
        query = '?'
        if group:
    ...
    def bulk_delete_users(self, ids=[]):
        self.logger.debug('bulk_delete_users module called')
    ...
    def get_status(self):
        self.logger.debug('get_status module called')
        api_path = '/status'
        headers = {'Content-Type' :  'application/json', 'cache-control': 'no-cache', 'cookie' : ''}
        headers.update(cookie=self.jsessionid)
        method = 'GET'
    ...
    def activate_status(self):
        self.logger.debug('activate_status module called')
    ...
    def _api_call(self, name, api_path, method, body=None, params=None, headers={}, cookies=None):
        ...
        while attempts < max_attempts:
            ...
            if api_path != '/authenticatedSession':
                self.logger.debug("calling self.authenticate")
                self.authenticate()
            ...
            if(attempts >= max_attempts):
                self.logger.error('Failed: Maxed out attempts')
    ...
    def modify_zscaler_user(self, zia, user, group, function):
        ...
        if len(existing_zscaler_user) == 1:
            ...
            if user['userPrincipalName'].lower() == get_updated_user_details['email'].lower():
                ...
                try:
                    #self.logger.info("Trying to Update Users Group Membership - Adding {} to {}".format(user['user'],group['name']))
                    #request_approval = input("Are you sure you wish to update the user groups? [y/N] : ")
                    #if request_approval.lower() == 'y': 
